            ...
            try
            {
                ...
                if (data.IsNotNull() && !data.HasTechnicalError)
                {
                    ...
                    if (data.HasBusinessError)
                    {
                        ...
                        logger.Log("Executions in Error : " + executionsInError.Count.ToString(), Category.Info, Priority.High);
                        logger.Log(data.GetFormattedErrors(ErrorMessageType.BusinessMessage), Category.Info, Priority.High);
                        logger.Log("Ignored Execution : " + executionsIgnored.Count.ToString(), Category.Info, Priority.High);
                        logger.Log("Successfully updated execution : " + oldExecutions.Count.ToString(), Category.Info, Priority.High);
                        logger.Log("Successfully newly imported execution : " + newExecutions.Count.ToString(), Category.Info, Priority.High);
                        logger.Log("Successfully updated Fx Hedge : " + modifiedFxHedge.Count.ToString(), Category.Info, Priority.High);
                    }
                    ...
                    else if (!data.ResultData.IsNullOrEmpty() && (!newExecutions.IsNullOrEmpty() || !oldExecutions.IsNullOrEmpty()))
                    {
                        ...
                        logger.Log("Import KTP has successfully completed", Category.Info, Priority.High);
                    ...
                    else
                    {
                        ...
                        //                MessageBoxButton.OK,
                        //                MessageBoxImage.Information);
                        logger.Log("No data imported", Category.Info, Priority.High);
                        foreach (var execution in executionsIgnored.Values)
                        {
                            logger.Log(execution.ExecutionCode + " - FxHedge : " + execution.FXHedge.Code + " not found ", Category.Info, Priority.High);
                        }
                    }
                }
                else
                {
                    ...
                    logger.Log(data.GetFormattedErrors(ErrorMessageType.TechnicalMessage), Category.Info, Priority.High);
                }

            }
            catch (Exception ex)
            {
                logger.Log(ex.InnerException.IsNotNull() ? ex.InnerException.Message : ex.Message,Category.Info, Priority.Medium);
                //DXMessageBox.Show("Import KTP has failed, please try again",
                //                                               "Error",
                //                                               MessageBoxButton.OK,
                //                                               MessageBoxImage.Error);
                logger.Log("Import KTP has failed, please try again", Category.Info, Priority.High);
            }
            finally
            {
